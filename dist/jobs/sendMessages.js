...
var fetchAccessToken = async (clientId, clientSecret) => {
  ...
  try {
    const {
      data: { access_token }
    } = await import_axios.default.post("https://us.auth.sinch.com/oauth2/token", data, config);
    return access_token;
  } catch {
...
var listContacts = async ({ projectId, accessToken }) => {
  try {
    ...
    const resp = await import_axios2.default.get(`https://us.conversation.api.sinch.com/v1/projects/${projectId}/contacts`, config);
